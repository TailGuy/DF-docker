...
logger: logging.Logger = logging.getLogger("telegraf_manager.telegraf_config_generator")
...
class TelegrafConfigGenerator:
    ...
    def __init__(self, 
                 ...
                 influxdb_url: str = "http://64.226.126.250:8086") -> None:
        ...
        self.opcua_endpoint: str = opcua_endpoint
    ...
    def generate_telegraf_config(self) -> None:
            ...
            with open(self.csv_file_path, 'r', encoding='utf-8') as csvfile:
                ...
                for row in reader:
                    ...
                    if all(key in row for key in ['NodeId', 'MQTTCustomName']):
                        ...
                        node_id_parts: List[str] = row['NodeId'].split(';')
                        if len(node_id_parts) != 2:
                            logger.warning(f"Skipping invalid NodeId format: {row['NodeId']}")
                            continue
                        
                        namespace: str = node_id_parts[0].replace('ns=', '')
                        identifier: str = node_id_parts[1].replace('s=', '')
                        mqtt_custom_name: str = row['MQTTCustomName']
                        
                        # Generate MQTT topic name using the MQTTCustomName and validate/sanitize it
                        mqtt_topic: str = f"telegraf/opcua/{mqtt_custom_name}"
                        ...
                        nodes.append({
                            ...
                            'identifier_type': 's',  # Assuming all are string type
                            'mqtt_topic': mqtt_topic
                        })
                        self._total_nodes_processed += 1
                    else:
                        missing_columns: List[str] = [key for key in ['NodeId', 'MQTTCustomName'] if key not in row or not row[key]]
        ...
        inputs = table()
        opcua_aot: List[Any] = cast(List[Any], aot())
        opcua = table()
        ...
        nodes_aot: List[Any] = cast(List[Any], aot())
        for node in nodes:
            node_table = table()
