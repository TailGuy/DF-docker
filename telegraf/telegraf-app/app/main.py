...
SHARED_OPCUA_CONFIG_PATH = "/app/shared_opcua/telegraf.conf"
SHARED_OPCUA_NODES_PATH = "/app/shared_opcua/nodes.csv"
...
class Settings(BaseSettings):
    telegraf_container_name: str = "telegraf-opcua"
    class Config:
        env_file = ".env"
...
async def root(request: Request):
    ...
    # backups = [f for f in os.listdir(SHARED_OPCUA_PATH) if f.startswith("nodes_backup_") and f.endswith(".csv")]
    # backups.sort(reverse=True) 

    backups = []
    for f in os.listdir(SHARED_OPCUA_PATH):
...
async def telegraf_config():
    ...
    logger.info(f"Reading config from shared path '{SHARED_OPCUA_CONFIG_PATH}'.")
    try:
        async with aiofiles.open(SHARED_OPCUA_CONFIG_PATH, "r") as f:
            content = await f.read()
        return content
    except FileNotFoundError:
        logger.error(f"Config file not found at '{SHARED_OPCUA_CONFIG_PATH}'.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def upload_telegraf_config(
    ...
    try:
        async with aiofiles.open(SHARED_OPCUA_CONFIG_PATH, "wb") as f:
            await f.write(content)
        logger.info(f"Successfully wrote new configuration to '{SHARED_OPCUA_CONFIG_PATH}'.")
    except Exception as e:
        logger.error(f"Failed to write configuration to '{SHARED_OPCUA_CONFIG_PATH}': {e}", exc_info=True)
        ...
        try:
            ...
            try:
                if check_csv_exists(SHARED_OPCUA_NODES_PATH):
                    ...
                    logger.warning(f"{SHARED_OPCUA_NODES_PATH} does not exist, skipping backup.")
            except:
                logger.error(f"Failed to back up existing CSV file at {SHARED_OPCUA_NODES_PATH}.", exc_info=True)
            with open(SHARED_OPCUA_NODES_PATH, 'wb') as nodes_csv:
                nodes_csv.write(content)
        except Exception as e:
            if nodes_csv_backed_up:
                os.rename(SHARED_OPCUA_PATH + f"/nodes_backup_{timestamp}.csv", SHARED_OPCUA_NODES_PATH)
            logger.error(f"Failed to write uploaded CSV content to {SHARED_OPCUA_NODES_PATH}: {e}", exc_info=True)
            return RedirectResponse(url="/?error=Failed to write uploaded CSV content", status_code=303)   

        generator = TelegrafConfigGenerator(
            csv_file_path=SHARED_OPCUA_NODES_PATH,
            output_file_path=SHARED_OPCUA_CONFIG_PATH,
            mqtt_broker=mqtt_broker,
            opcua_endpoint=opcua_endpoint,
            influxdb_url=influxdb_url
        )
        generator.run()  # Generates and writes the config to SHARED_OPCUA_CONFIG_PATH
...
async def export_nodes_csv():
    ...
    try:
        return FileResponse(SHARED_OPCUA_NODES_PATH, filename="nodes.csv")
    except:
        logger.error(f"Failed to export nodes CSV file from {SHARED_OPCUA_NODES_PATH}.")
...
async def get_csv_data(request: Request):
    if not os.path.exists(SHARED_OPCUA_NODES_PATH):
        raise HTTPException(status_code=404, detail="CSV file not found.")
    
    with open(SHARED_OPCUA_NODES_PATH, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
...
class SettingsTest(BaseSettings):
    telegraf_container_name: str = "telegraf-mqtt"
    class Config:
        env_file = ".env"
        env_file_encoding = "utf-8"
...
def get_telegraf_test_container() -> docker.models.containers.Container:
    """Dependency to get the Telegraf-Test container object."""
    logger.info(f"Attempting to get container '{settings_test.telegraf_container_name}'...")
    try:
        container = docker_client.containers.get(settings_test.telegraf_container_name)
        logger.info(f"Successfully retrieved container '{container.name}' (ID: {container.short_id}).")
        return container
    except docker.errors.NotFound:
        logger.error(f"Container '{settings_test.telegraf_container_name}' not found.")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Container '{settings_test.telegraf_container_name}' not found"
        )
    except docker.errors.APIError as e:
        logger.error(f"Docker API error while getting container: {e}", exc_info=True)
        raise HTTPException(
...
# Reuse get_telegraf_status_details for telegraf-test (no changes needed)

@app.get("/telegraf-test", response_class=HTMLResponse, tags=["UI"])
async def root_test(request: Request):
    """Renders the main dashboard for Telegraf-Test."""
    fastapi_status = {
        "status": "Running",
    ...
    container = get_telegraf_test_container()
    telegraf_status = get_telegraf_status_details(container)
    
    backups = []
    if os.path.exists(SHARED_MQTT_PATH):
        for file in os.listdir(SHARED_MQTT_PATH):
            if file.startswith("nodes-test_backup_") and file.endswith(".csv"):
                file_path = os.path.join(SHARED_MQTT_PATH, file)
                date = datetime.datetime.fromtimestamp(os.path.getmtime(file_path)).isoformat()
                backups.append({"filename": file, "date": date})
    backups.sort(key=lambda x: x["date"], reverse=True)
    
    file_exists = os.path.exists(SHARED_MQTT_NODES_PATH)
    
    success_message = request.query_params.get("success")
    error_message = request.query_params.get("error")
    
    return templates.TemplateResponse("index_test.html", {
        "request": request,
...
@app.get("/telegraf-test/status", tags=["Telegraf-Test"])
async def get_telegraf_status_test(
    container: docker.models.containers.Container = Depends(get_telegraf_test_container)
):
    """Returns the status details of the Telegraf-Test container."""
    return get_telegraf_status_details(container)

@app.get("/telegraf-test/config", response_class=PlainTextResponse, tags=["Telegraf-Test"])
async def view_telegraf_config_test():
    """Returns the current Telegraf-Test configuration file as plain text."""
    logger.info("Fetching Telegraf-Test configuration file.")
    try:
        async with aiofiles.open(SHARED_MQTT_CONFIG_PATH, mode='r') as f:
            content = await f.read()
        return content
    except FileNotFoundError:
        logger.error(f"Configuration file '{SHARED_MQTT_CONFIG_PATH}' not found.")
...
@app.post("/telegraf-test/change-interval", tags=["Telegraf-Test"])
async def change_agent_interval_test(
    request: Request,
    new_interval: str = Form(...),
    container: docker.models.containers.Container = Depends(get_telegraf_test_container)
):
    """Changes the agent interval in the Telegraf-Test config and restarts the container."""
    logger.info(f"Received request to change agent interval to '{new_interval}'.")
    try:
        if not re.match(r'^\d+[s|m|h]$', new_interval):
            raise ValueError("Invalid interval format. Use e.g., '30s', '5m'.")
        
        async with aiofiles.open(SHARED_MQTT_CONFIG_PATH, mode='r') as f:
            content = await f.read()
        
        new_content = re.sub(r'interval\s*=\s*".*?"', f'interval = "{new_interval}"', content)
        
        async with aiofiles.open(SHARED_MQTT_CONFIG_PATH, mode='w') as f:
            await f.write(new_content)
        
        logger.info("Agent interval updated successfully.")
    except FileNotFoundError:
        logger.error(f"Configuration file '{SHARED_MQTT_CONFIG_PATH}' not found.")
        raise HTTPException(status_code=404, detail="Configuration file not found.")
    except ValueError as ve:
        logger.error(f"Invalid interval: {ve}")
        raise HTTPException(status_code=400, detail=str(ve))
    except Exception as e:
        logger.error(f"Error updating configuration file: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error updating configuration: {e}")
    
    try:
        container.restart(timeout=30)
        logger.info(f"Container '{container.name}' restarted successfully after interval change.")
    except Exception as e:
        logger.error(f"Failed to restart container '{container.name}': {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to restart container: {e}")
    
    if "application/x-www-form-urlencoded" in request.headers.get("content-type", ""):
        return RedirectResponse(url="/telegraf-test", status_code=status.HTTP_303_SEE_OTHER)
    return {"success": True, "message": f"Agent interval changed to {new_interval} and container restarted."}

@app.post("/telegraf-test/restart", tags=["Telegraf-Test"])
async def restart_telegraf_test(
    request: Request,
    container: docker.models.containers.Container = Depends(get_telegraf_test_container)
):
    """Restarts the Telegraf-Test container."""
    logger.info(f"Received restart request for container '{container.name}'.")
    try:
        container.restart(timeout=30)
    ...
    if "application/x-www-form-urlencoded" in request.headers.get("content-type", ""):
        return RedirectResponse(url="/telegraf-test", status_code=status.HTTP_303_SEE_OTHER)
    return {"success": True, "message": "Telegraf-Test container restarted successfully."}

@app.post("/telegraf-test/stop", tags=["Telegraf-Test"])
async def stop_telegraf_test(
    request: Request,
    container: docker.models.containers.Container = Depends(get_telegraf_test_container)
):
    """Stops the Telegraf-Test container, breaking any restart loops."""
    logger.info(f"Received stop request for container '{container.name}'.")
    try:
        container.stop(timeout=10)
        logger.info(f"Container '{container.name}' stopped successfully.")
    ...
    if "application/x-www-form-urlencoded" in request.headers.get("content-type", ""):
        return RedirectResponse(url="/telegraf-test", status_code=status.HTTP_303_SEE_OTHER)
    return {"success": True, "message": "Telegraf-Test container stopped."}

@app.post("/telegraf-test/start", tags=["Telegraf-Test"])
async def start_telegraf_test(
    request: Request,
    container: docker.models.containers.Container = Depends(get_telegraf_test_container)
):
    """Starts the Telegraf-Test container."""
    logger.info(f"Received start request for container '{container.name}'.")
    try:
        container.start()
        logger.info(f"Container '{container.name}' started successfully.")
    ...
    if "application/x-www-form-urlencoded" in request.headers.get("content-type", ""):
        return RedirectResponse(url="/telegraf-test", status_code=status.HTTP_303_SEE_OTHER)
    return {"success": True, "message": "Telegraf-Test container started."}

@app.post("/telegraf-test/upload-csv", tags=["Telegraf-Test"])
async def upload_csv_for_config_test(
    request: Request,
    file: UploadFile = File(...),
    mqtt_broker: str = Form("tcp://mosquitto:1883"),
    influxdb_url: str = Form("http://influxdb:8086"),
    ...
    if not content:
        safe_error = quote("Empty file")
        return RedirectResponse(url=f"/telegraf-test?error={safe_error}", status_code=303)

    try:
        nodes_csv_backed_up = False
        try:
            logger.info(f"Writing uploaded CSV content to {SHARED_MQTT_NODES_PATH}.")
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            try:
                if os.path.exists(SHARED_MQTT_NODES_PATH):
                    os.rename(SHARED_MQTT_NODES_PATH, SHARED_MQTT_PATH + f"/nodes-test_backup_{timestamp}.csv")
                    nodes_csv_backed_up = True
                else:
                    logger.warning(f"{SHARED_MQTT_NODES_PATH} does not exist, skipping backup.")
            except:
                logger.error(f"Failed to back up existing CSV file at {SHARED_MQTT_NODES_PATH}.", exc_info=True)
            with open(SHARED_MQTT_NODES_PATH, 'wb') as nodes_csv:
                nodes_csv.write(content)
        except Exception as e:
            if nodes_csv_backed_up:
                os.rename(SHARED_MQTT_PATH + f"/nodes-test_backup_{timestamp}.csv", SHARED_MQTT_NODES_PATH)
            logger.error(f"Failed to write uploaded CSV content to {SHARED_MQTT_NODES_PATH}: {e}", exc_info=True)
            return RedirectResponse(url="/telegraf-test?error=Failed to write uploaded CSV content", status_code=303)   

        generator = TelegrafMQTTConfigGenerator(
            csv_file_path=SHARED_MQTT_NODES_PATH,
            output_file_path=SHARED_MQTT_CONFIG_PATH,
            mqtt_broker=mqtt_broker,
            influxdb_url=influxdb_url
        )
        generator.run()  # Generates and writes the config to SHARED_MQTT_CONFIG_PATH
    ...
    return RedirectResponse(url="/telegraf-test?success=csv_uploaded", status_code=303)

@app.get("/export_nodes_csv_test")
async def export_nodes_csv_test():
    logger.info("Exporting nodes-test CSV file.")
    try:
        return FileResponse(SHARED_MQTT_NODES_PATH, filename="nodes-test.csv")
    except:
...
async def get_csv_data_test(request: Request):
    ...
    with open(SHARED_MQTT_NODES_PATH, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        headers = next(reader, None)  # Get headers; None if empty
        rows = list(reader)  # Get all data rows
